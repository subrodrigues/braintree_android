...
public class TPAnimatedViewPager extends ViewPager {

	public static final String TAG = "TPViewPager";
	...
	public enum TransitionEffect {
		Standard, Tablet, CubeIn, CubeOut, FlipVertical, FlipHorizontal, FadePink, Stack, ZoomIn, ZoomOut, RotateUp, RotateDown, Accordion
	}
	...
	public TPAnimatedViewPager(Context context) {
		this(context, null);
	}

	@SuppressWarnings("incomplete-switch")
	public TPAnimatedViewPager(Context context, AttributeSet attrs) {
		super(context, attrs);
		setClipChildren(false);
		// now style everything!
		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.TPAnimatedViewPager);
		int effect = ta.getInt(R.styleable.TPAnimatedViewPager_vptyle, 0);
		String[] transitions = getResources().getStringArray(R.array.tp_viewpager_effects);
		setTransitionEffect(TransitionEffect.valueOf(transitions[effect]));
		setFadeEnabled(ta.getBoolean(R.styleable.TPAnimatedViewPager_fadeEnabled, false));
		setOutlineEnabled(ta.getBoolean(R.styleable.TPAnimatedViewPager_outlineEnabled, false));
		setOutlineColor(ta.getColor(R.styleable.TPAnimatedViewPager_outlineColor, Color.WHITE));
		switch (mEffect) {
			case Stack:
			case ZoomOut:
				setFadeEnabled(true);
	...
	private void wrapWithOutlines() {
		for (int i = 0; i < getChildCount(); i++) {
			View v = getChildAt(i);
			if (!(v instanceof TPOutlineContainer)) {
				removeView(v);
	...
	private View wrapChild(View child) {
		if (!mOutlineEnabled || child instanceof TPOutlineContainer)
			return child;
		TPOutlineContainer out = new TPOutlineContainer(getContext());
		out.setLayoutParams(generateDefaultLayoutParams());
		child.setLayoutParams(new TPOutlineContainer.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
		out.addView(child);
		return out;
	}
	...
	protected void animateFade(View left, View right, float positionOffset, int positionOffsetPixels) {
		if (mState != State.IDLE) {
			if (right != null) {
				manageLayer(right, true);
	...
	protected void animateOutline(View left, View right) {
		if (!(left instanceof TPOutlineContainer))
			return;
		if (mState != State.IDLE) {
			if (left != null) {
				manageLayer(left, true);
				((TPOutlineContainer) left).setOutlineAlpha(1.0f);
			}
			if (right != null) {
				manageLayer(right, true);
				((TPOutlineContainer) right).setOutlineAlpha(1.0f);
			}
		} else {
			if (left != null)
				((TPOutlineContainer) left).start();
			if (right != null)
				((TPOutlineContainer) right).start();
		}
	}
