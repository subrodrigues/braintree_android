...
public class TPExpandablePanel extends LinearLayout {
	...
	public TPExpandablePanel(Context context) {
		this(context, null);
	}
	 ...
	 * 'content'
	 */
	public TPExpandablePanel(Context context, AttributeSet attrs) {
		super(context, attrs);
		mListener = new DefaultOnExpandListener();

		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.TPExpandablePanel, 0, 0);

		// How high the content should be in "collapsed" state
		mCollapsedHeight = (int) a.getDimension(R.styleable.TPExpandablePanel_collapsedHeight, 0.0f);

		// How long the animation should take
		mAnimationDuration = a.getInteger(R.styleable.TPExpandablePanel_animationDuration, 500);

		isOpened = a.getBoolean(R.styleable.TPExpandablePanel_isOpened, false);

		int handleId = a.getResourceId(R.styleable.TPExpandablePanel_handle, 0);
		...
		int contentId = a.getResourceId(R.styleable.TPExpandablePanel_content, 0);
		if (contentId == 0) {
	...
	// TPExpandablePanel programmatically
	...
	protected void onFinishInflate() {
		...
		// mHandle.setOnTouchListener(PanelTouch);
	}
	...
	private class ExpandAnimation extends Animation {
		...
		public ExpandAnimation(int startHeight, int endHeight) {
			...
			mEndHeight = startHeight;
			mDeltaHeight = endHeight - startHeight;
			setInterpolator(new DecelerateInterpolator());
			this.setAnimationListener(expandAnimationListener);
		}
		...
		private AnimationListener expandAnimationListener = new AnimationListener() {
			...
			public void onAnimationEnd(Animation animation) {
				...
				if (mEndHeight > mStartHeight) {
					mListener.finishExpanding();

				} else {
					mListener.finishCollapsing();
				}
			}
		};
	}
