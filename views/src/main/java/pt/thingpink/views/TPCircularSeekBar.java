...
public class TPCircularSeekBar extends View {
	...
	private static final int DEFAULT_CIRCLE_SECONDARY_PROGRESS_COLOR = Color.argb(235, 74, 138, 255);
	...
	private static final int DEFAULT_POINTER_DRAWABLE = 0;
	...
	/**
	 * {@code Paint} instance used to draw the active secondary circle
	 * (represents secondary progress).
	 */
	private Paint mCircleSecondaryProgressPaint;
	...
	/**
	 * {@code Paint} instance used to draw the glow from the active secondary
	 * circle.
	 */
	private Paint mCircleSecondaryProgressGlowPaint;
	...
	/**
	 * The width of the progress (in pixels).
	 */
	private float mProgressStrokeWidth;

	/**
	 * The width of the secondary progress (in pixels).
	 */
	private float mSecondaryProgressStrokeWidth;
	...
	/**
	 * Holds the color value for {@code mCircleSecondaryProgressPaint} before
	 * the {@code Paint} instance is created.
	 */
	private int mCircleSecondaryProgressColor = DEFAULT_CIRCLE_SECONDARY_PROGRESS_COLOR;
	...
	/**
	 * Distance (in degrees) that the current secondary progress makes up in the
	 * circle.
	 */
	private float mSecondaryProgressDegrees;
	 ...
	 */
	private Path mCircleSecondaryProgressPath;
	...
	/**
	 * Last Progress value that this CircularSeekBar represented.
	 */
	 ...
	 */
	private boolean shouldLockBar = true;

	/**
	 * Used for when the user moves beyond the start of the circle when moving
	 * counter clockwise. Makes it easier to hit the 0 progress mark.
	 */
	private boolean shouldLockEndBar = false;
	...
	private int pointerDrawable;

	private Context context;

	private boolean shouldDrawDefaultPointer = false;
	...
	private void initAttributes(TypedArray attrArray) {
		mCircleXRadius = (float) (attrArray.getFloat(R.styleable.TPCircularSeekBar_circle_x_radius, DEFAULT_CIRCLE_X_RADIUS) * DPTOPX_SCALE);
		mCircleYRadius = (float) (attrArray.getFloat(R.styleable.TPCircularSeekBar_circle_y_radius, DEFAULT_CIRCLE_Y_RADIUS) * DPTOPX_SCALE);
		mPointerRadius = (float) (attrArray.getFloat(R.styleable.TPCircularSeekBar_pointer_radius, DEFAULT_POINTER_RADIUS) * DPTOPX_SCALE);
		mPointerHaloWidth = (float) (attrArray.getFloat(R.styleable.TPCircularSeekBar_pointer_halo_width, DEFAULT_POINTER_HALO_WIDTH) * DPTOPX_SCALE);
		mPointerHaloBorderWidth = (float) (attrArray.getFloat(R.styleable.TPCircularSeekBar_pointer_halo_border_width, DEFAULT_POINTER_HALO_BORDER_WIDTH) * DPTOPX_SCALE);
		mCircleStrokeWidth = (float) (attrArray.getFloat(R.styleable.TPCircularSeekBar_circle_stroke_width, DEFAULT_CIRCLE_STROKE_WIDTH) * DPTOPX_SCALE);
		mProgressStrokeWidth = (float) (attrArray.getFloat(R.styleable.TPCircularSeekBar_progress_stroke_width, DEFAULT_CIRCLE_STROKE_WIDTH) * DPTOPX_SCALE);
		mSecondaryProgressStrokeWidth = (float) (attrArray.getFloat(R.styleable.TPCircularSeekBar_secondary_progress_stroke_width, DEFAULT_CIRCLE_STROKE_WIDTH) * DPTOPX_SCALE);
		pointerDrawable = (int) (attrArray.getResourceId(R.styleable.TPCircularSeekBar_pointer_drawable, DEFAULT_POINTER_DRAWABLE));

		if (pointerDrawable == DEFAULT_POINTER_DRAWABLE) {
			shouldDrawDefaultPointer = true;
		}

		String tempColor = attrArray.getString(R.styleable.TPCircularSeekBar_pointer_color);
		if (tempColor != null) {
			try {
		...
		tempColor = attrArray.getString(R.styleable.TPCircularSeekBar_pointer_halo_color);
		if (tempColor != null) {
			try {
		...
		tempColor = attrArray.getString(R.styleable.TPCircularSeekBar_circle_color);
		if (tempColor != null) {
			try {
		...
		tempColor = attrArray.getString(R.styleable.TPCircularSeekBar_circle_progress_color);
		if (tempColor != null) {
			try {
		...
		tempColor = attrArray.getString(R.styleable.TPCircularSeekBar_circle_secondary_progress_color);
		if (tempColor != null) {
			try {
				mCircleSecondaryProgressColor = Color.parseColor(tempColor);
			} catch (IllegalArgumentException e) {
				mCircleSecondaryProgressColor = DEFAULT_CIRCLE_SECONDARY_PROGRESS_COLOR;
			}
		}

		tempColor = attrArray.getString(R.styleable.TPCircularSeekBar_circle_fill);
		if (tempColor != null) {
			try {
		...
		mPointerAlphaOnTouch = attrArray.getInt(R.styleable.TPCircularSeekBar_pointer_alpha_ontouch, DEFAULT_POINTER_ALPHA_ONTOUCH);
		if (mPointerAlphaOnTouch > 255 || mPointerAlphaOnTouch < 0) {
			mPointerAlphaOnTouch = DEFAULT_POINTER_ALPHA_ONTOUCH;
		}

		mMax = attrArray.getInt(R.styleable.TPCircularSeekBar_max, DEFAULT_MAX);
		mProgress = attrArray.getInt(R.styleable.TPCircularSeekBar_progress, DEFAULT_SECONDARY_PROGRESS);
		mSecondaryProgress = attrArray.getInt(R.styleable.TPCircularSeekBar_secondaryProgress, DEFAULT_PROGRESS);
		mCustomRadii = attrArray.getBoolean(R.styleable.TPCircularSeekBar_use_custom_radii, DEFAULT_USE_CUSTOM_RADII);
		mMaintainEqualCircle = attrArray.getBoolean(R.styleable.TPCircularSeekBar_maintain_equal_circle, DEFAULT_MAINTAIN_EQUAL_CIRCLE);
		mMoveOutsideCircle = attrArray.getBoolean(R.styleable.TPCircularSeekBar_move_outside_circle, DEFAULT_MOVE_OUTSIDE_CIRCLE);

		// Modulo 360 right now to avoid constant conversion
		mStartAngle = ((360f + (attrArray.getFloat((R.styleable.TPCircularSeekBar_start_angle), DEFAULT_START_ANGLE) % 360f)) % 360f);
		mEndAngle = ((360f + (attrArray.getFloat((R.styleable.TPCircularSeekBar_end_angle), DEFAULT_END_ANGLE) % 360f)) % 360f);
	...
	private void initPaints() {
		...
		mCircleSecondaryProgressPaint.setStrokeWidth(mSecondaryProgressStrokeWidth);
		...
		mCircleSecondaryProgressGlowPaint = new Paint();
		mCircleSecondaryProgressGlowPaint.set(mCircleSecondaryProgressPaint);
		mCircleSecondaryProgressGlowPaint.setMaskFilter(new BlurMaskFilter((5f * DPTOPX_SCALE), BlurMaskFilter.Blur.NORMAL));
		...
		if (pointerDrawable != DEFAULT_POINTER_DRAWABLE) {
			...
			pointerBitmap = BitmapFactory.decodeResource(resources, pointerDrawable);
			pointerWidth = pointerBitmap.getWidth() / 2;
			pointerHeight = pointerBitmap.getWidth() / 2;
		}
	...
	/**
	 * Calculate the degrees that the secondary progress represents. Also called
	 * the sweep angle. Sets mSecondaryProgressDegrees to that value.
	 */
	private void calculateSecondaryProgressDegrees() {
		mSecondaryProgressDegrees = mSecondaryProgress / mMax * 360f;
		// mSecondaryProgressDegrees = mPointerPosition - mStartAngle; //
		// Verified
		mSecondaryProgressDegrees = (mSecondaryProgressDegrees < 0 ? 360f + mSecondaryProgressDegrees
				: mSecondaryProgressDegrees); // Verified
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawPath(mCircleSecondaryProgressPath, mCircleSecondaryProgressGlowPaint);
		...
		if (shouldDrawDefaultPointer == true) {
			...
			matrix.setRotate(((float) mProgress) / mMax * 360, pointerWidth, pointerHeight);
			matrix.postTranslate(mPointerPositionXY[0] - pointerWidth, mPointerPositionXY[1]
					- pointerHeight);
	...
	/**
	 * Get the progress of the TPCircularSeekBar.
	 * 
	 * @return The progress of the TPCircularSeekBar.
	 */
	...
	/**
	 * Set the progress of the TPCircularSeekBar. If the progress is the same,
	...
	public int getSecondaryProgress() {
		int secondaryProgress = Math.round((float) mMax
                * mSecondaryProgressDegrees / mTotalCircleDegrees);
		return secondaryProgress;
	}

	/**
	 * Set the secondary progress of the CircularSeekBar. If the secondary
	 * progress is the same, then any listener will not receive a
	 * onProgressChanged event.
	 * 
	 * @param secondaryProgress
	 *            The secondary progress to set the CircularSeekBar to.
	 */
	public void setSecondaryProgress(int secondaryProgress) {
		if (mSecondaryProgress != secondaryProgress) {
			mSecondaryProgress = secondaryProgress;
			if (mOnCircularSeekBarChangeListener != null) {
				mOnCircularSeekBarChangeListener.onSecondaryProgressChanged(this, secondaryProgress, false);
			}
	...
	private void setSecondaryProgressBasedOnAngle(float angle) {
		mPointerPosition = angle;
		calculateSecondaryProgressDegrees();
		mSecondaryProgress = Math.round((float) mMax
                * mSecondaryProgressDegrees / mTotalCircleDegrees);
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (pointerDrawable != DEFAULT_POINTER_DRAWABLE) {
			mCircleHeight = (float) height / 2f - pointerHeight;
			mCircleWidth = (float) width / 2f - pointerWidth;
		} else {
		...
		switch (event.getAction()) {
			case MotionEvent.ACTION_DOWN:
				...
				if (((touchEventRadius >= innerRadius) && (touchEventRadius <= outerRadius))
					...
					lockAtEnd = false;
					lockAtStart = false;
				} else if (cwDistanceFromStart > mTotalCircleDegrees) { // If
																		// the
																		// user
																		// is
																		// touching
					...
					return false;
						...
						&& (touchEventRadius <= outerRadius)) { // If the user
																// is touching
																// near the
																// circle
			...
			case MotionEvent.ACTION_MOVE:
				if (mUserIsMovingPointer) {
					...
					if (lockAtStart && !mIsMovingCW
							&& (ccwDistanceFromStart > 360)) {
						lockAtStart = false;
					}
					if (lockAtEnd && mIsMovingCW && (cwDistanceFromEnd > 0)) {
						lockAtEnd = false;
					}
					if (shouldLockEndBar) {
						lockAtEnd = true;
	...
	private void init(AttributeSet attrs, int defStyle) {
		final TypedArray attrArray = getContext().obtainStyledAttributes(attrs, R.styleable.TPCircularSeekBar, defStyle, 0);
	...
	public TPCircularSeekBar(Context context) {
		super(context);
		this.context = context;
		init(null, 0);
	}

	public TPCircularSeekBar(Context context, AttributeSet attrs) {
		super(context, attrs);
		this.context = context;
		init(attrs, 0);
	}

	public TPCircularSeekBar(Context context, AttributeSet attrs, int defStyle) {
	...
	protected Parcelable onSaveInstanceState() {
		...
		state.putInt("SECONDARY_PROGRESS", mSecondaryProgress);
		state.putInt("mCircleColor", mCircleColor);
		state.putInt("mCircleProgressColor", mCircleProgressColor);
		state.putInt("mCircleSecondaryProgressColor", mCircleSecondaryProgressColor);
	...
	protected void onRestoreInstanceState(Parcelable state) {
		...
		mSecondaryProgress = savedState.getInt("SECONDARY_PROGRESS");
		mCircleColor = savedState.getInt("mCircleColor");
		mCircleProgressColor = savedState.getInt("mCircleProgressColor");
		mCircleSecondaryProgressColor = savedState.getInt("mCircleSecondaryProgressColor");
	...
	public interface OnCircularSeekBarChangeListener {
		...
		public abstract void onProgressChanged(TPCircularSeekBar circularSeekBar, int progress, boolean fromUser);

		public abstract void onSecondaryProgressChanged(TPCircularSeekBar circularSeekBar, int secondaryProgress, boolean fromUser);
	...
	/**
	 * Sets the circle secondary progress color.
	 * 
	 * @param color
	 *            the color of the circle progress
	 */
	public void setCircleSecondaryProgressColor(int color) {
		mCircleSecondaryProgressColor = color;
		mCircleSecondaryProgressPaint.setColor(mCircleSecondaryProgressColor);
		invalidate();
	}

	/**
	 * Gets the circle secondary progress color.
	 * 
	 * @return An integer color value for the circle progress
	 */
	public int getCircleSecondaryProgressColor() {
		return mCircleSecondaryProgressColor;
	}
	...
	public void setMax(int max) {
		if (!(max <= 0)) { // Check to make sure it's greater than zero
			...
			if (max <= mSecondaryProgress) {
				mSecondaryProgress = 0; // If the new max is less than current
										// progress, set progress to zero
				if (mOnCircularSeekBarChangeListener != null) {
					mOnCircularSeekBarChangeListener.onSecondaryProgressChanged(this, mSecondaryProgress, false);
				}
			}
			mMax = max;
	...
	/**
	 * @return the shouldLockBar
	 */
	public boolean isShouldLockBar() {
		return shouldLockBar;
	}

	/**
	 * @param shouldLockBar
	 *            the shouldLockBar to set
	 */
	public void setShouldLockBar(boolean shouldLockBar) {
		this.shouldLockBar = shouldLockBar;
	}

	/**
	 * @return the shouldLockEndBar
	 */
	public boolean isShouldLockEndBar() {
		return shouldLockEndBar;
	}

	/**
	 * @param shouldLockEndBar
	 *            the shouldLockEndBar to set
	 */
	public void setShouldLockEndBar(boolean shouldLockEndBar) {
		this.shouldLockEndBar = shouldLockEndBar;
	}
